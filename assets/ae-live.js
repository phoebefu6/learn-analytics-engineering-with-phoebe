/* learn-analytics-engineering-with-phoebe - live dbt warehouse engine
   Every .sqlbox on a page is a REAL, editable SQL script run against a fresh
   copy of the Bazaar warehouse (assets/bazaar-raw.js + assets/dbt-models.js) on DuckDB - an
   ...
   Markup a page uses (same shape as the learn-sql course):
     <div class="sqlbox" data-caption="optional line under the box"
          data-setup="built" data-seed="none">
       <pre class="sql-src">SELECT * FROM customers;</pre>
     </div>
   data-seed="none"  -> start from an EMPTY database (CREATE-from-scratch demos)
   data-setup="built" -> run the project's compiled dbt models first, so the
                         staging views and marts exist (see dbt-models.js)
      ...
      loadEngine(function (msg) {
        ...
        return enqueue(function () {
          return resetSql(eng.conn).then(function () {
            var pre = "";
            if (seeded) pre += (window.BAZAAR_RAW || "");
            if (seeded && setupName && window.AE_SETUPS && window.AE_SETUPS[setupName]) {
              pre += "\n" + window.AE_SETUPS[setupName];
            }
